...
//
//  https://github.com/nicklockwood/IKeychain
//
...
@implementation NSObject (IKeychainPropertyListCoding)

- (id)IKeychain_propertyListRepresentation
{
    return self;
}
...
#if !IKeychain_USE_NSCODING

@implementation NSNull (IKeychainPropertyListCoding)
...
- (id)IKeychain_propertyListRepresentation
{
    ...
    for (id obj in self)
    {
        id value = [obj IKeychain_propertyListRepresentation];
...
+ (instancetype)defaultKeychain
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        sharedInstance = [[IKeychain alloc] initWithService:bundleID
                                                 accessGroup:nil];
    });
          ...
          accessGroup:(NSString *)accessGroup
{
    return [self initWithService:service
                   ...
                   accessibility:IKeychainAccessibleWhenUnlocked];
}

- (id)initWithService:(NSString *)service
          accessGroup:(NSString *)accessGroup
        accessibility:(IKeychainAccess)accessibility
{
    if ((self = [super init]))
    {
...
- (NSData *)dataForKey:(id)key
{
    ...
    {
		NSLog(@"IKeychain failed to retrieve data for key '%@', error: %ld", key, (long)status);
	}
...
- (BOOL)setObject:(id)object forKey:(id)key
{
    ...
    if (object && !data)
    {
        data = [NSPropertyListSerialization dataWithPropertyList:[object IKeychain_propertyListRepresentation]
    ...
    NSAssert(!object || (object && data), @"IKeychain failed to encode object for key '%@', error: %@", key, error);
    ...
    if (data)
    {
        ...
        {
            NSLog(@"IKeychain failed to store data for key '%@', error: %ld", key, (long)status);
            return NO;
        }
    }
    else if (self[key])
    {
        ...
        if (status != errSecSuccess)
        {
            NSLog(@"IKeychain failed to delete data for key '%@', error: %ld", key, (long)status);
            return NO;
        }
    }
    return YES;
...
- (id)objectForKey:(id)key
{
    ...
    if (data)
    {
        ...
        if (!object)
        {
             NSLog(@"IKeychain failed to decode data for key '%@', error: %@", key, error);
        }
        return object;
    }
    else
