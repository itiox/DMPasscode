//
//  IPasscodeInternalViewController.m
//  Pods
//
...
#import "IPasscodeInternalField.h"
#import "IPasscodeConfig.h"

@interface IPasscodeInternalViewController () <UITextFieldDelegate>
@end

@implementation IPasscodeInternalViewController {
    __weak id<IPasscodeInternalViewControllerDelegate> _delegate;
...
- (void)viewDidLoad {
    ...
    for (int i = 0; i < 4; i++) {
        IPasscodeInternalField* field = [[IPasscodeInternalField alloc] initWithFrame:CGRectMake(((itemWidth + space) * i), 0, itemWidth, itemWidth) config:_config];
...
- (void)editingChanged:(UITextField *)sender {
    for (int i = 0; i < sender.text.length; i++) {
        IPasscodeInternalField* field = [_textFields objectAtIndex:i];
        NSRange range;
        range.length = 1;
        range.location = i;
...
- (void)reset {
    for (IPasscodeInternalField* field in _textFields) {
        field.text = @"";
    }
    _input.text = @"";
}
